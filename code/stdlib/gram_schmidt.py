...
def gram_schmidt_columns(X, zero_threshold = zero_threshold):
...
def sub_gram_schmidt(X_to_adjust, X_rest, zero_threshold = zero_threshold):
    ...
    X_rest = gram_schmidt_columns(X_rest)#necessary or else gram schmidt procedure fails 
    X_rest_sums = sum(X_rest**2)
    X_rest = X_rest / np.sqrt(X_rest_sums)
    X_to_adjust = X_to_adjust/np.linalg.norm(X_to_adjust)


    for v in X_rest.T:
        X_to_adjust -= np.dot(X_to_adjust.ravel(), v)*v[:,np.newaxis]
