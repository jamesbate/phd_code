import unittest
import numpy
from . import initialconditions as ic
...
class InitialConditionsTestCase(unittest.TestCase):
    def test_gaussian(self):
        ...
        for x0, k0, sigma, fin in pars:
            ...
            self.assertTrue(numpy.abs(F.norm()-1)<eps)
            self.assertTrue(numpy.abs(F.diagexpvalue(K)-k0)<eps)
            f = F.fft()
            self.assertTrue(numpy.abs(f.norm()*numpy.sqrt(dx)-1)<eps)
            ev_x0 = f.diagexpvalue(X)*dx
            ev_x0square = f.diagexpvalue(X**2)*dx
            std_x0 = numpy.sqrt(ev_x0square - ev_x0**2)
            self.assertTrue(numpy.abs(ev_x0-x0)<eps)
            self.assertTrue(numpy.abs(std_x0-sigma)<eps)
    ...
    def test_coherentstate(self):
        ...
        for alpha, N in pars:
            ...
            self.assertTrue(numpy.abs(sv.norm()-1)<eps)
            a = numpy.diag(numpy.sqrt(numpy.arange(1,N)),1)
            at = numpy.diag(numpy.sqrt(numpy.arange(1,N)),-1)
            ev_a = sv.expvalue(a)
            ev_n = sv.expvalue(numpy.dot(at,a))
            self.assertTrue(numpy.abs(ev_a - alpha)<eps)
            self.assertTrue(numpy.abs(ev_n - numpy.abs(alpha)**2)<eps) 
