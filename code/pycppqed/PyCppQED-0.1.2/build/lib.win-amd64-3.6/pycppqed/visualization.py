...
def statevector(sv, x=None, y=None, re=False, im=False, abs=True, show=True,
    ...
    *Arguments*
    ...
    indices = numpy.array(list(map(bool, (re, im, abs))))
...
def expvaluecollection(evc, show=True, **kwargs):
    ...
    *Arguments*
    ...
    if evc.subsystems:
        import pylab
        for sysname, data in evc.subsystems.items():
            pylab.figure()
...
def compare_expvaluecollections(coll1, coll2, show=True, **kwargs):
    ...
    *Arguments*
    ...
    for i in range(len(s1)):
        pylab.figure()
        _compare_expvaluesubsystems(list(s1.values())[i], list(s2.values())[i],
                                        show=False, **kwargs)
        title = "%s vs. %s" % (list(s1.keys())[i], list(s2.keys())[i])
        if hasattr(pylab, "suptitle"): # For old versions not available.
