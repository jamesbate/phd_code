##----------------------------PREAMBLE----------------------------------##
from stdlib import TricsDataObject, data_loader, FitTemplate, fit_rabi_twoions, plot_error_joint, rabi_flops_LD_carrier, fit_constant, full_Rabi, full_Rabi_sq, PlotTemplate, full_Rabi_av, full_Rabi_nooptpump, fit_rabi_twoions_nooptpump, generalised_Rabi
...
data_folders = ["215747/"]
data_folders_BSB = ["213228/", "212643/", "212815/", "213021/"]
...
fit_temp.parameters.add('n_av_0',0.5, vary = False, min = 0)
fit_temp.parameters.add('n_av_1',4.5, vary = False)
fit_temp.parameters.add('n_av_2',4.5, vary = False, expr = "n_av_1")
fit_temp.parameters.add('detuning', 0, vary = False)
fit_temp.parameters.add('amplitude', 1, vary = False)
fit_temp.parameters.add('Rabi', 1.1, vary = True)
fit_temp.parameters.add('phase', 0, vary = False)

#results = fit_temp.do_minimisation(pulse_length, np.array([probs[:,0]]).T, errorbars = np.array([dprobs[:,0]]).T, weights = 1/np.array([dprobs_weights[:,0]]).T)
results = fit_temp.do_minimisation(pulse_length, me, weights = 1/dme_weights,m = [0,0], total_modes = 3, n_start_index = 3, ld_start_index = 0, sum_limit = 20)
params = fit_temp.get_opt_parameters()
lab = r"$\Omega_0: $" + str(round(params['Rabi'],3)) + r"$(1)MHz$"+"\nn_av: " + str(round(params['n_av_0'],1))+ r"$(1)$"
fit_temp.plot_fit(pulse_length, me, errorbars = dme, label=lab, colour_index=0, title = "Carrier Flops", xlabel = r"Pulse length $\mu s$", ylabel = "Mean Excitation", error_lines = False, ax = ax,m = [0,0], total_modes = 3, n_start_index = 3, ld_start_index = 0, sum_limit = 20)
fit_temp.print_fit_result()
print(fit_temp.fit_result_error_dict)
#plt.savefig(save_dir+'Carrier.png')
plt.show()
...
#     #free parameters
#     fit_temp.parameters.add('n_av',0.75, vary = False)
...
for i in range(3,4):
    ...
    fit_temp.parameters.add('dicke_factor_1', 0.047, vary = False)
    fit_temp.parameters.add('dicke_factor_2', 0.047, vary = False)
    fit_temp.parameters.add('n_av_0',2.13503300, vary = True)
    fit_temp.parameters.add('n_av_1',10.17432975, vary = True)
    fit_temp.parameters.add('n_av_2',10.17432975, vary = True, expr = "n_av_1")
    ...
    results = fit_temp.do_minimisation(pulse_length, me, m = [-1,0], total_modes = 3, n_start_index = 3, ld_start_index = 0, sum_limit = 20)
    fit_temp.print_fit_result()
    params = fit_temp.get_opt_parameters()
    ...
    args_0 = [fit_temp.fit_result_params, {'m': [-1,0], "total_modes": 3, "n_start_index": 3, "ld_start_index": 0, "sum_limit": 20}]
    args_1 = [params1,{'m': [-1,0], "total_modes": 3, "n_start_index": 3, "ld_start_index": 0, "sum_limit": 20}]
    args_2 = [params2, {'m': [-1,0], "total_modes": 3, "n_start_index": 3, "ld_start_index": 0, "sum_limit": 20}]
    lab = r"$\Omega_0: $" + str(round(params['Rabi'],3)) + "(3)MHz"+"\n"+r"$n_{av}$: " + str(round(params['n_av_0'],2))+ r"$("+str(n_av_lower)+', '+str(n_av_upper)+")$"
    fit_temp.plot_fit_errorcurves(pulse_length, me, dme,args_0, args_1, args_2, color = plot_template.colours[i], label=lab, title = "RSB Flops", xlabel = r"Pulse length $\mu s$", ylabel = "Mean Excitation", fill = True, ax = ax)
...
#         error_n = np.std(n_av)/rsb_pulse_length.size
#         print(error_n)
...
#     plot_error_joint(rsb_pulse_length, rsb_me/bsb_me, final_error,rsb_pulse_length*0 + m, rsb_pulse_length*0 + m+mean_error, rsb_pulse_length*0 + m-mean_error, fill = True,title = "RSB/BSB Ratio Plot", xlabel = r"Pulse length $\mu s$",ylabel = r"$\frac{\mathrm{RSB Mean Excitation}}{\mathrm{RSB Mean Excitation}}$", label = "n_av: {}({})".format(round(n_av_mean,2), int(error_n*100)),color = plot_template.colours[k] )
