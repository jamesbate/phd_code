from lib import TricsDataObject, data_loader, create_hist_plot, fit_PMT_histogram
...
results = fit_PMT_histogram(phases, np.array([probs[:,0]]).T, np.array([dprobs[:,0]]).T,labels = ['p2'], initial_fit = initial_fits, ground = True)
