from lib import TricsDataObject, data_loader, FitTemplate, fit_rabi_twoions, plot_error_joint, rabi_flops_LD_carrier, fit_constant, full_Rabi, full_Rabi_sq, PlotTemplate, full_Rabi_av, full_Rabi_nooptpump, fit_rabi_twoions_nooptpump
...
save_dir = 'C:/Users/James/OneDrive - OnTheHub - The University of Oxford/phd/images/trics/temperature_plots_redo_SS_ampfree/'
...
savefiles = ['blue_temperature_fits_ss.png', 'red_temperature_fits_ss.png']
...
# params = fit_temp.get_opt_parameters()
# lab = r"$\Omega_0: $" + str(round(params['Rabi'],2)) + r"$MHz$"
# fit_temp.plot_fit(pulse_length[:-1], np.array([probs[:-1,1]]).T, errorbars = None, ax = ax, label=lab, colour_index=3, n_ions = 2, title = "Carrier Flops - Camera", xlabel = r"Pulse length $\mu s$", ylabel = r"$P_{\uparrow}$")
# fit_temp.print_fit_result()
# print(fit_temp.fit_result_error_dict)
...
#     fit_temp = FitTemplate(fit_rabi_twoions_nooptpump, log_dir=None)
...
#     #free parameters
#     fit_temp.parameters.add('n_av',0.42, vary = False)
#     fit_temp.parameters.add('Rabi', 0.46, vary = False)
#     fit_temp.parameters.add('phase', 0, vary = True)
#     results = fit_temp.do_minimisation(pulse_length, np.array([probs[:,2]]).T, errorbars = np.array([dprobs[:,2]]).T, weights = 1/np.array([dprobs_weights[:,0]]).T, dn = 1, pop_state = 0)
#     fit_temp.print_fit_result()
#     params = fit_temp.get_opt_parameters()
...
#     args_0 = [fit_temp.fit_result_params, {'dn': 1, 'pop_state': 0}]
#     args_1 = [params1, {'dn': 1, 'pop_state': 0}]
#     args_2 = [params2, {'dn': 1, 'pop_state': 0}]
...
#     fit_temp.plot_fit_errorcurves(pulse_length, np.array([probs[:,2]]).T, np.array([dprobs[:,2]]).T,args_0, args_1, args_2, color = plot_template.colours[i], label=lab, title = "BSB Flops", xlabel = r"Pulse length $\mu s$", ylabel = r"$P_{\downarrow\downarrow}$", fill = True)
#     print(fit_temp.fit_result_error_dict)
...
for i in range(3,5):
    ...
    fit_temp.parameters.add('Rabi', 0.46, vary = False)
    fit_temp.parameters.add('phase', 0, vary = True)
    results = fit_temp.do_minimisation(pulse_length, np.array([probs[:,2]]).T, errorbars = np.array([dprobs[:,2]]).T, weights = 1/np.array([dprobs_weights[:,2]]).T, dn = -1, pop_state = 0)
    fit_temp.print_fit_result()
    params = fit_temp.get_opt_parameters()
    ...
    fit_temp.plot_fit_errorcurves(pulse_length, np.array([probs[:,2]]).T, np.array([dprobs[:,2]]).T,args_0, args_1, args_2, color = plot_template.colours[i], label=lab, title = "RSB Flops", xlabel = r"Pulse length $\mu s$", ylabel = r"$P_{\downarrow\downarrow}$", fill = True)
    print(fit_temp.fit_result_error_dict)
...
#         rsb_probs = rsb_probs[:,2]
#         rsb_dprobs = rsb_dprobs[:,2]
#         bsb_probs, bsb_dprobs = bsb_data.get_probs(thresholds = [18,75], nmax = 150, allow_zero_error = True)
#         bsb_probs = bsb_probs[:,2]
#         bsb_dprobs = bsb_dprobs[:,2]
#         #do fit 
...
#         mask2 = final_error < rsb_probs/bsb_probs
#         mask = np.logical_and(mask1, mask2)
#         mask3 = final_error + rsb_probs/bsb_probs < 1 + 0*final_error
...
#     print(m/(1-m))
#     #rsb_result = fit_temp.do_minimisation(rsb_pulse_length, rsb_probs/bsb_probs, weights = 1/final_error)
...
#     #plt.savefig(save_dir+'RatioPlot_noerror_postselect'+str(k)+'.png', dpi = 1000)
