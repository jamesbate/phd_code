...
# detection time
det_time=self.set_variable("float","det_time",1000.000000,1,2e7)
...
#Doppler Cooling and repumping
ttl_pulse("854 sw",repump_length,is_last=False)
OpticalPumping(doppler_length)
#PMTDetection(doppler_length)
seq_wait(0.1,start_time = doppler_length)

#Do a Raman pulse while 854 and 866 ON and count
setTTLOn("854 sw",0,is_last=False)
setTTLOn("866 sw",0,is_last=True)
rf_pulse(raman_length, 0, ion=1, transition_param='729_Test', is_last=False, address=0)
ttl_pulse("31",raman_length,is_last=True)

setTTLOff("854 sw",0,is_last=False)
setTTLOff("866 sw",0,is_last=True)
seq_wait(0.1)

for i in range (20):
    RamanPulse(repump_length,doppler_length)
...
#seq_wait(wait_time)
#ttl_pulse("31",raman_length,is_last=True)
#setTTLOff("854 sw",0,is_last=True)
...
ttl_pulse("854 sw",det_time,is_last=False)
ttl_pulse("Pi397",det_time,is_last=False)
ttl_pulse("dp397",det_time,is_last=False)
ttl_pulse("866 sw",det_time,is_last=True)
PMTDetection(det_time)
