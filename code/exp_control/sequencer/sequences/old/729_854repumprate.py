...
#qqq=self.set_variable("float","qqqq",20.000000,1,2e5)
repump_length=self.set_variable("float","repump_length",20.000000,1,2e5)
repump_time=self.set_variable("float","repump_time",20.000000,1,2e5)
...
<SEQUENCE>
ttl_pulse("StartSeq",10,is_last=False)
...
#ttl_pulse("866 sw",doppler_length*0.1,is_last=True)

#optical pumping
if opt_pumping:
	seq_wait(wait_time)
#	ttl_pulse("854 sw",pump_length,is_last=False)
	OpticalPumping(pump_length)
	#ttl_pulse("Sigma397",pump_length,is_last=False)
	#ttl_pulse("dp397",pump_length,is_last=False)
	#ttl_pulse("866 sw",pump_length*1.2,is_last=True)
	seq_wait(wait_time)

if sb_cool_com:
	SBCooling2(SBCool_time)
	#rf_pulse(SBCool_time, 0, ion=1, transition_param='sideband_cool', is_last=False, address=1)
	#ttl_pulse("866 sw",SBCool_time+50,is_last=False)
	#ttl_pulse("854 sw",SBCool_time+50,is_last=True)
...
if p729_pulse:
	...
	#seq_wait(wait_time)
	seq_wait(wait_time2)
	#ttl_pulse("729 sw",pulse_length,is_last=True)
	rf_pulse(pulse_length, 0, ion=1, transition_param='729_Carrier', is_last=True, address=1)
	#rf_729(pulse_length, 0, "729_Test", is_last=True)
	#seq_wait(wait_time)
ttl_pulse("854 sw",repump_time,is_last=True)
#detection
...
setTTLOn("Pi397",0,is_last=False)
setTTLOn("866 sw",0,is_last=True)
