...
if abs(dummy_var - 4) < 0.0001:
    pulse_delay = 217.7  # Delay between TTL and optical pulse (measured)
    ttl_pulse("30", 100, start_time=0.0, is_last=False)
    ttl_pulse("Al PI", 100, start_time=pulse_delay + 20, is_last=True)

seq_wait(1.)

if pulse_729:
    rf_729(1, 0, "clock1", is_last=True)
    seq_wait(1.)
