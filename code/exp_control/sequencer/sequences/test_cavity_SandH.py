...
setTTLOn("17",0.0,is_last=False) #test channel for cavity-lock sample and hold
setTTLOn("9",0.0,is_last=True) #806 SP going to ion cavity
InitPulse()
...
for i in range(int(n_loops)):
    ...
    DopplerCooling(det_time, is_last=True)
    ...
    if switchSH:
        setTTLOff("17",0.0,is_last=True) #test channel for cavity-lock sample and hold
        seq_wait(delaySH)
    if switch806SP:
        setTTLOff("9",0.0,is_last=True) #806 SP going to ion cavity
    RamanPulse(raman_length)
    seq_wait(delay)
    setTTLOn("9",0.0,is_last=True) #806 SP going to ion cavity
    seq_wait(delaySH)
    setTTLOn("17",0.0,is_last=True) #test channel for cavity-lock sample and hold
