...
<VARIABLES>
activate_scantype=self.set_variable("bool","activate_scantype",0)
scantype1=self.set_variable("float","scantype1",0,0,1e5)



# initializing the ion to the s state with 854 laser
MeasPoints=self.set_variable("float","MeasPoints",1.000000,1,2e5)
n_loops=self.set_variable("float","n_loops",1,1,500000)

repump_length=self.set_variable("float","repump_length",60.000000,1,2e5)

raman_length=self.set_variable("float","raman_length",100.000000,1,2e5)
# Doppler cooling
doppler_length=self.set_variable("float","doppler_length",1000.000000,1,2e5)

#optical pumping with 397 sigma
opt_pumping=self.set_variable("bool","opt_pumping",1)
pump_length=self.set_variable("float","pump_length",10.000000,1,2e5)
...
SBCool_time_rad=self.set_variable("float","SBCool_time_rad",2500.000000,1,2e5)
...
radial729probe=self.set_variable("bool","radial729probe",0)
...
# detection time
det_time=self.set_variable("float","det_time",1500.000000,0.01,2e7)
...
setTTLOff("729SPswitch",0,is_last=True) #turns on axial beam

if activate_scantype:
    if scantype1%3==0:
        do_raman=1
        probe729_pulse=0
    if scantype1%3==1:
...
for i in range(int(n_loops)):
    ...
    #DopplerCooling(200, is_last=True)
    #PMTDetection(doppler_length)
    seq_wait(0.1)
    #if opt_pumping:
        #ttl_pulse(["Sigma397","dp397"],pump_length,is_last=False)
